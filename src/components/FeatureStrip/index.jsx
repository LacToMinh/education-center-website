      ...
      <div className="mx-auto w-full py-8 pt-[50px]">
        ...
        <ul className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {items.map((it, idx) => (
            <li
              key={idx}
              className="flex items-center gap-3 sm:gap-4 p-3 sm:py-4 sm:px-4 rounded-xl border border-gray-400 bg-white shadow-sm hover:shadow-md transition-all duration-200"
            >
              {/* Icon */}
              <span className="flex h-6 w-6 sm:h-12 sm:w-12 items-center justify-center rounded-full border-2 border-[#001F5D] text-[#FEA500] bg-white">
                {it.icon}
              </span>
              ...
              <div className="leading-tight">
                ...
                <div className="mt-[2px] text-[8px] sm:text-[15px] text-slate-500">
                  {it.desc}
                </div>
              </div>
            </li>
