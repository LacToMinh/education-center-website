      ...
      <div className="relative overflow-hidden rounded-3xl">
        ...
        <div className="absolute inset-0 p-6 md:p-10 flex flex-col justify-end">
          ...
          <div className="mt-4 flex flex-wrap gap-3 text-white/95">
            {cfg.meta?.hours && <Chip icon={<FiClock />}>{cfg.meta.hours}</Chip>}
            {cfg.meta?.level && <Chip icon={<FiBookOpen />}>{cfg.meta.level}</Chip>}
            {cfg.meta?.classSize && (
                ...
                {(cfg.syllabus ?? []).map((w, i) => (
                  <div key={i} className="rounded-xl border p-4">
                    <div className="flex items-center justify-between">
                      ...
                      {w.hours && (
                        <div className="text-sm text-slate-500">{w.hours}</div>
                      )}
                    </div>
                    ...
                    <div key={i}>
                      ...
                      >
                        <div className="overflow-hidden">
                          <p className="pb-3 text-[15px] text-slate-700">{f.a}</p>
                        </div>
                      </div>
                    </div>
                  );
        ...
        <aside className="md:col-span-1">
          <div className="md:sticky md:top-20">
            <div className="rounded-2xl p-5 shadow-lg border bg-white">
              ...
              {cfg.pricing?.note && (
                <p className="mt-2 text-sm text-slate-600">{cfg.pricing.note}</p>
              )}
